...
class CondGAN:
    ...
    def global_hook_fn(self):
        ...
        # if self.global_step == 82:
        #     self.netg.optimizer.param_groups[1]['lr'] = self.hyperparameters['lr_g']
        #     self.netd.optimizer.param_groups[1]['lr'] = self.hyperparameters['lr_d']
...
class Generator(nn.Module):
    def __init__(self, hyperparameters, log_dir, gpu_id, run_id):
        ...
        self.conv2 = nn.ConvTranspose2d(self.hyp['p3'], 1, (55, 1), bias=True)  # 2.nd arg DIM
    ...
    def calc_gen_ce_loss(self):
        ...
        ce_loss = torch.nn.functional.binary_cross_entropy_with_logits(out.view(-1), self.fake_labels.float())
        return ce_loss
...
class Discriminator(nn.Module):
    def __init__(self, hyperparameters, gpu_id):
        ...
        # self.conv2 = nn.Conv2d(self.hyp['q3'], self.hyp['q4'], 1, bias=False)
        # self.nonlin2 = nn.Sequential(
        #     *[nn.BatchNorm2d(self.hyp['q4']), nn.LeakyReLU(self.hyp['lrelu_d'])] if self.hyp['bd2'] else [
        #         nn.LeakyReLU(self.hyp['lrelu_d']), ])

        # self.fc = nn.Linear(self.hyp['q4'], 1, bias=False)
