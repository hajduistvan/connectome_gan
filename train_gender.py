...
from libs.models import convnet_gender as convnet
import numpy as np
...
from libs.models.ConditionalFCWGAN_gender import ConditionalFCWGAN_gender
from libs.models.AugNet import AugNet
from libs.utils.plots import plot_learning_curves_gender
...
def main(config, run_name, mode):
    ...
    if mode == '---':
        cnn = convnet.ConnectomeConvNet(
            config,
            val_loader=None,
            exp_name='rnd_init_freezing',
            rnd_init=True
        ).cuda()
        path = os.path.join('frozen_inits',config.CLASS_FROZEN_INIT_FILENAME)
    ...
    if mode == 'exp':
        ...
        for size_idx in range(len(train_sizes)):
            ...
            pure_cnn_val_acc_sublist = []
            pure_cnn_dev_acc_sublist = []
            for n in range(num_cnn_runs):
                ...
                best_val, best_acc = cnn.run_train(train_class_loader)
                dev_error, dev_acc = cnn.evaluate(dev_loader)
                pure_cnn_val_sublist.append(best_val)
                pure_cnn_dev_sublist.append(dev_error)
                pure_cnn_val_acc_sublist.append(best_acc)
                pure_cnn_dev_acc_sublist.append(dev_acc)

            pure_cnn_val_losses.append(pure_cnn_val_sublist)
            pure_cnn_dev_losses.append(pure_cnn_dev_sublist)
            pure_cnn_val_acces.append(pure_cnn_val_acc_sublist)
            pure_cnn_dev_acces.append(pure_cnn_dev_acc_sublist)

            gan = eval(config.GAN_MODEL)(config, train_dataset, val_loader, )
            gan.netg.cuda()
            gan.netd.cuda()
            best_val, best_acc = gan.train()

            gan_dev_err, gan_dev_acc = gan.cnn.evaluate(dev_loader)
            mixed_cnn_dev_losses.append(gan_dev_err)
            mixed_cnn_val_losses.append(best_val)
            mixed_cnn_dev_acces.append(gan_dev_acc)
            mixed_cnn_val_acces.append(best_acc)
            np.savez(
                ...
                pure_val_acc=pure_cnn_val_acces,
                pure_dev_acc=pure_cnn_dev_acces,
                mix_val_acc=mixed_cnn_val_acces,
                mix_dev_acc=mixed_cnn_dev_acces,
                last_size_acc=str(train_sizes[size_idx]+val_sizes[size_idx])
            )
