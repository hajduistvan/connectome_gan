...
from libs.models.convnet_gender import ConnectomeConvNet
...
class ConditionalFCWGAN_gender:
    def __init__(
            self,
            config,
            train_dataset,
    ...
    def validate(self):
        ...
        val_loss, val_acc = self.cnn.run_train(gan_val_loader)
        self.writer.add_scalar("gan_valid_loss", val_loss, self.global_step)
        self.writer.add_scalar("gan_valid_acc", val_acc, self.global_step)
...
class Generator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        labels = torch.zeros(labels.size()[0], 2).cuda().scatter_(1, labels, 1).view(*labels.shape, -1)
        x = torch.cat([self.block0(x), self.lab_0(labels).view(-1,self.dim,1,1)], 1)
    ...
    def generate_rnd_labels(self):
        batch_size = self.config.BATCH_SIZE.GAN.TRAIN
        return (torch.randint(0,2,(batch_size,))).type(torch.long)
    ...
    def generate_training_images(self, num_images):
        ...
        for i in range(num_images // b):
            ...
            samples = self(noise, labels[i * b:(i + 1) * b])
    ...
    def visualize_gen_images(self, global_step, exp_name):
        ...
        labels = (torch.randint(0,2,(num_images,))).type(torch.long).cuda().requires_grad_(False)
...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        self.lab_0 = nn.Sequential(
            nn.ConvTranspose2d(2, self.dim, (1, self.h)),
        )
        # concat
        self.block_lab_0 = nn.Sequential(
    ...
    def forward(self, data, labels):
        labels = labels.view(-1, 1).long()
        labels = torch.zeros(labels.size()[0], 2).cuda().scatter_(1, labels, 1).view(-1,2,1,1)
